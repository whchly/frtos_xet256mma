...
void CAN_Transmit(uint8 AChn)
{
    ...
    if (p->CANTAAK.Byte & interruptBufferBit)           /* transmission aborted */
    {
        ...
        if ((p_Buffer + buffer)->Status == QUEUED2)     /* aborted due to higher priority queued message buffer */
        {                                                                 
    ...
    */
    ...
    for (buffer = 0; buffer < NoOfMB[AChn_Transformed]; buffer++)
    {
            ...
            || ((RX_FrameType != DATAFRAME) && (bufferMode == AUTOTXDF)))
        {
            ...
            MO_ID8_CAN_Ptr = (uint8*)&((MSCANIdentifier[AChn_Transformed]+MB_ID_Ref)->data);

            if (RX_IdentifierType == CAN_STANDARD)
            {
                if (MB_ID_Type == CAN_STANDARD)                                 /* MB_ID_Type matches */
                {
                    if (  (p->CANRXIDR0.Byte           == MO_ID8_CAN_Ptr[0])    /* ID10 to ID3 match */
                        && ((p->CANRXIDR1.Byte & 0xE0) == MO_ID8_CAN_Ptr[1]))   /* ID2 to ID0 match */
                    {
            ...
            else                                                                /* MO_IdentiferType == EXTENDED */
            {
                if (MB_ID_Type != CAN_STANDARD)                                 /* MB_ID_Type matches */
                {
                    if (    (p->CANRXIDR0.Byte         == MO_ID8_CAN_Ptr[0])    /* ID28 to ID21 match */
                        &&  (p->CANRXIDR1.Byte         == MO_ID8_CAN_Ptr[1])    /* ID20 to ID15 match */
                        &&  (p->CANRXIDR2.Byte         == MO_ID8_CAN_Ptr[2])    /* ID14 to ID7 match */
                        && ((p->CANRXIDR3.Byte & 0xFE) == MO_ID8_CAN_Ptr[3]))   /* ID6 to ID0 match */
                    {
                        match = TRUE;                                           /* match found */
                        break;
                    }
